...
bool LL1parse(){            // LL(1)驱动程序 , 返回真假值表示是否出现了语法错误   P107
    ...
    while(!symbolStack.empty()){
       ...
       if( tmpSymbol <= MaxTerminalValue){  // 如果是终极符   则匹配
            if(Match(tmpSymbol))
                ...
                sprintf(strError,"syntax error :token 类型不匹配,unexpected tokentype= '%s' name= %s found at line %d,\n\
            ...
            if( n == -1){  // 错误编号, 报错
                sprintf(strError,"syntax error :符号表对应错误 unexpected tokentype= '%s' name= %s found at line %d.\n",
                                        getTokenStr(currentToken.type).c_str(),currentToken.name.c_str(),
    ...
    // 匹配 结束符
    if( !Match(DOT)){
        sprintf(strError,"syntax error :token类型不匹配 unexpected token found at line %d,Expected Dot\n",currentToken.lineNum);
        insertError(strError);
        return false;
    }else if(!ReadOneToken(currentToken)){
        sprintf(strError,"syntax error : 缺少EOF the token EOF is missed in tokenList\n");
        insertError(strError);
        return false;
    }
    ...
    if( Match(ENDFILE) )
        ...
        sprintf(strError,"syntax error : 缺少文件终止符 unexpected token '%s' found at line %d,Expected EOF\n"
...
int priosity(TokenType op){
    ...
    if (op == TIMES || op == OVER) return 4;
        ...
        sprintf(strError, "没有该操作符/该操作符不具备优先级 No such operator and no priosity for it.syntax error : unexpected token '%s' found at line %d,Expected Operator\n"
...
void process33() {  // VarDecList ::= TypeId EQ VarIdList SEMI VarDecMore
...
// 输出语法错误信息
void outputParserLL1Errors(const char errorFileName[])
{
    FILE * fn = fopen(errorFileName,"a");
    for(string tmp : errors)           // 遍历Vector并输
...
TreeNode* getLL1Tree() {
    ...
    getAllPredict();   //initPredict();

    if(tokens.size()==0) return NULL; // 如果词法分析错误, 那么不进行语法分析

    bool errorflag=LL1parse();
    const char fileName[] = "D:/CODE_Qt/txt/ll1syntaxTree.txt";
    ...
    if ( errorflag==1 ){
        ...
        rt=NULL;
        outputParserTree(rt , fileName);
        outputParserLL1Errors(errorFileName);
    }
